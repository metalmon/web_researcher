...
from openrouter_cascade import generate_cascading_completion, calculate_requests_needed
...
def local_generate_completion(messages, temperature=0.4, response_format=None):
    kwargs = {}
    if response_format:
...
def unified_generate_completion(role, task, content, temperature=0.1, response_format=None, is_final=False, local_only=False):
    ...
    return generate_cascading_completion(
        messages,
        local_completion_fn=local_generate_completion,
        is_final=is_final,
        temperature=temperature,
...
def validate_llm_contacts_by_regex(llm_contacts, text):
...
def enrich_contacts_with_llm(text, local_only=True):
    ...
    try:
        ...
        if match:
            ...
            llm_contacts = []
    # Валидация LLM-ответа по регуляркам
    llm_contacts = validate_llm_contacts_by_regex(llm_contacts, text)
    # ВРЕМЕННАЯ ОТЛАДКА: если нет ни одного telegram, вывести предупреждение и кусок текста
    if not any(c.get("telegram") for c in llm_contacts):
        ...
        print(text[:500])
    # Объединяем дубли и собираем максимум информации по каждому контакту
    merged = merge_contacts([llm_contacts])
    # Оставляем только контакты, у которых есть хотя бы одно из ключевых полей
    def has_key_field(c):
    ...
    # Фильтрация виртуальных контактов
...
def crawl_and_extract_contacts(url, limit=10, local_only=True):
    ...
    page_num = 1
    llm_requests = 0  # Счётчик реальных запросов к LLM
    # Словари: идентификатор -> найдено ли ФИО
    email_has_name = dict()
    phone_has_name = dict()
    ...
    for item in tqdm(data, desc="Извлечение контактов"):
        ...
        if content:
            ...
            page_num += 1
            regex_contacts = extract_contacts_regex(content)
            # Фильтруем только те, для которых еще не найдено ФИО
            def not_enriched(c):
            ...
            if filtered_contacts:
                llm_requests += 1
                contacts = enrich_contacts_with_llm(content, local_only=local_only)
                # После enrich_contacts_with_llm обновляем словари: если найдено ФИО, помечаем идентификатор как обогащённый
                llm_keys = set()
                for c in contacts:
                    if c.get("email"):
                ...
                # Теперь помечаем как enriched только телефоны, которые были отправлены, но не вернулись из LLM
...
def summarize_contacts_with_llm(contacts, local_only=True):
    prompt = (
    ...
    # Всегда используем remote (OpenRouter) если force_remote=True
    return generate_cascading_completion(
        messages,
        local_completion_fn=local_generate_completion,
        is_final=True,
...
def select_best_contact_pages(links, limit, local_only=True, force_remote=False):
    """
    Выбирает наиболее релевантные для поиска контактов страницы из списка ссылок с помощью LLM.
    ...
    response = generate_cascading_completion(
        messages,
        local_completion_fn=local_generate_completion,
        is_final=True,
        temperature=0.1,
...
if __name__ == "__main__":
    init(autoreset=True)
    #print(Fore.CYAN + Style.BRIGHT + "=== Извлечение данных с сайта ===" + Style.RESET_ALL)
    ...
    parser.add_argument('-o', '--output', type=str, help='Файл для сохранения контактов (CSV)', default='contacts.csv')
    ...
    # --- Новый pipeline: карта сайта + LLM-выбор страниц ---
    print(Fore.BLUE + "\nПолучаем карту сайта через Firecrawl..." + Style.RESET_ALL)
    site_map = app.map_url(url)
    all_links = site_map.links

    # --- Формируем список страниц для анализа ---
    pages_to_parse = [url]  # всегда начинаем с главной
    ...
    if not all_links or (len(all_links) == 1 and all_links[0].rstrip('/') == url.rstrip('/')):
        ...
        # --- Fallback: если нет ссылок и нет контактов на главной, пробуем Playwright ---
        regex_contacts = extract_contacts_regex(content)
        if not all_links and not regex_contacts:
            ...
            if not all_links and not regex_contacts_pw:
                ...
                sys.exit(0)
            # Обновляем content и pages_to_parse для дальнейшего pipeline
            content = content_pw
            if not all_links:
                pages_to_parse = [url]
    else:
        ...
        best_links = select_best_contact_pages(other_links, n, local_only=local_only, force_remote=force_remote)
        if contact_url:
    ...
    for link in pages_to_parse:
        print("  ", link)
    # --- Batch scrape выбранных страниц ---
    ...
    for page_url in pages_to_parse:
        ...
        # Добавить все элементы crawl_status.data в all_data
        all_data.extend(crawl_status.data)
    # --- Дальнейший pipeline: извлечение контактов из контента ---
    all_contacts = []
    os.makedirs('outputs', exist_ok=True)
    page_num = 1
    llm_requests = 0
    ...
    merged_contacts = finalize_contacts_format(merged_contacts)
    merged_contacts = [c for c in merged_contacts if not is_virtual_contact(c)]
    # --- Финальный LLM-отбор лучших контактов ---
    ...
    summary = summarize_contacts_with_llm(merged_contacts, local_only=local_only and not force_remote)
    try:
        if isinstance(summary, dict):
            best_contacts = summary.get("contacts", [])
